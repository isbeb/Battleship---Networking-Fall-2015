...
bool battle_phase(Board player, Board opponent)
{
    ...
    do
    {
		...
		// insert method 
		bool shot = opponent.received_shot(x,y); // Info sent from server later saying whether the players shot was a hit(true) or miss(false)
		mark_shot( x, y, player, shot); //marks shot onto players board

		//Display of your shot 
		shot_display(x, y, player, shot, "\nPlayer");

		//oponenets turn
		auto_shoot(x, y, opponent); // remove in network version and replace with shot from oponnent

		shot = player.received_shot(x,y); // where player would check if the shot was a hit or miss
		mark_shot( x, y, opponent, shot); // replace with a method passing whether or not it was a hit or miss
...
int main(){
	...
	{
		Board playerboard;
		Board opponentboard; // remove in network version

		system("cls");

		auto_set(opponentboard); // remove in network version
