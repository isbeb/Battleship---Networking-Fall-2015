/*
General operation notes for those who aren't aware:
Run the client and server in separate projects. Run server first. Make sure they're not run in debug mode.
*/
...
int main() {
	...
	char *senderBuffer = "Hey, I want to play Battleship!";
	...
	// Wait until the server confirms our entrance
	do {
	...
	do {
		iResult = recv(connectSocket, newBuffer, receiverBufferLength, 0);
		if (iResult > 0) {
			cout << newBuffer << endl;
		}
	} while (iResult > 0);

	cout << "At this point, the players would start the game." << endl;
